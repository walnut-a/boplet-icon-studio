...
先读取 `version.json` 的 `version`，向用户确认/沿用数据目录，完整加载 SKILL.md 后再声明 `ICON_STUDIO_SKILL_LOADED`。以下为宿主执行标准，不是面向用户的新业务 CLI。

macOS / shell（替换绝对路径与包版本）：

```sh
ICON_STUDIO_SKILL_LOADED=0.1.0-dev.1 node /absolute/skill/runtime/start.mjs /absolute/authorized-library
```
...
WebMCP 优先：在支持的浏览器使用 `icon_studio_v3_get_workflow` 等工具。浏览器不支持时退回上述本地路径，所有业务操作照常。允许在属性区域简短说明支持 WebMCP 的浏览器效率更高，不阻断用户。
