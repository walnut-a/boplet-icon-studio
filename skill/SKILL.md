...
1. **项目库**：复用已授权的项目库目录 → `connect_library(create:false)` → `list_projects` → 打开目标项目，或为新产品调用 `create_project`。一个库容纳多个产品项目，每个项目有多个独立方案；新设计不等于新库，不为产品、任务或对话另建库目录。仅首次使用或用户明确要求独立库时创建新库，目录选择见 [运行与恢复](references/runtime.md)。不得把方案当项目。
...
8. **交付**：明确项目/方案/图标/变体范围，`prepare_export` → 最终 `get_export` → `read_artifact`。SVG 必须与当前编译哈希相符。按钮使用同一流程；源快照不等于迁移包。
   **交给其他 Agent**：工具和“复制给 Agent”按钮共用 `get_agent_handoff`。方案列表复制 scope:scheme；结构详情与应用场景右栏均复制 scope:variant，对应右栏明确显示的当前尺寸；模式切换保持该变体，场景样本仍展示整个图标的用途且不保留几何选区。指令包含授权目录、确切 ID、主线信息和源定位，不包含令牌或临时端口。草稿／备选也可交接，不自动确认。接收 Agent 先加载 Skill 并遵循 HTML 同意流程，获本地权限后读取当前源与 SVG；哈希是复制时快照，内容变化时说明，不回退新修改。不擅自改动源项目。跨设备或无权限时请求目录访问或导出文件，不假装已读取。
