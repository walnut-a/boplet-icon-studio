...
import { readRelease, repository } from './release-skill.js';
...
// 官网索引使用仓库现名；原候选和已发布附件保持不变。
const downloadRelease = release ? { ...release, repository,
  url: `https://github.com/${repository}/releases/download/${release.tag}/${release.filename}` } : null;
...
try {
  ...
  await writeFile(join(downloads, 'release.json'), JSON.stringify(downloadRelease || { skillId: 'make-product-icons', version: report.version, buildId: report.buildId, stage: 'preview', url: `https://boplet.app/downloads/${filename}`, sha256, bytes: bytes.length }, null, 2));
