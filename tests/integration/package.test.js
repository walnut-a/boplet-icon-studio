...
test('白名单包脱离仓库依赖启动、离线重开，卸载不删除独立数据', async t => {
  ...
  assert.equal(version.version, JSON.parse(await readFile('package.json', 'utf8')).version); assert.equal(version.schemaRevision, 3);
  ...
  const first = await start(); const health=await fetch(first.receipt.url+'/health').then(r=>r.json());assert.equal(health.ready,true);assert.equal(health.schemaRevision,3);assert.equal(health.buildId,report.buildId);
