...
const bindingPath = a => `${schemePath(a)}/usages/${a.iconId}/${a.variantId}.json`;
const historyRoot = a => `${projectPath(a)}/history/${a.schemeId}/${a.iconId}`;
...
export function registerMaintenanceOperations(define) {
  ...
  define('get_usage_bindings', { description: '读取当前变体已绑定用途，没有数据返回空数组。', input: object(v), data: object({ bindings: bindingSchema }), handler: async (r, a) => { await readMatrix(r, a); return { bindings: await readOptional(r, bindingPath(a), []) }; } });
  define('set_usage_bindings', { description: '按对话明确用途绑定固定模板，不生成评价。', input: input({ ...v, bindings: bindingSchema }), data: object({ bindings: bindingSchema }), mutates: true, persists: true, handler: async (r, a) => {
    const matrix = await readMatrix(r, a); requireValue(matrix.variants.find(v => v.variantId === a.variantId)); await r.storage.writeJson(bindingPath(a), a.bindings); return { bindings: a.bindings };
  } });
  define('get_context_preview', { description: '展开图标全部绑定用途、固定尺寸对比和反色；样本标明源变体，不继承隐藏结构选区。', input: object(v, Object.keys(i)), data: object({ samples: array(object({ kind: enumeration('usage','size_comparison','inverse'), presetId: text, name: text, displaySize: number, sourceSize: number, variantId: identity.variantId, inverse: bool, svg: longText })) }), handler: async (r, a) => {
    ...
    for (const variant of matrix.variants.filter(v => v.status === 'active' && (!a.variantId || v.variantId === a.variantId))) {
      const source = { ...a, variantId: variant.variantId }; const bindings = await readOptional(r, bindingPath(source), []); if (!bindings.length) continue;
  ...
  define('record_feedback', { description: '保留用户原文反馈与目标内容版本。', input: input({ ...v, comment: text }), data: object({ feedback: feedbackSchema }), mutates: true, persists: true, handler: async (r, a) => {
    ...
    await r.storage.writeJson(`${projectPath(a)}/feedback/${feedback.feedbackId}.json`, feedback); return { feedback };
  } });
  define('list_feedback', { description: '读取明确目标的用户反馈，不添加主观评价。', input: object(v), data: object({ items: array(feedbackSchema) }), handler: async (r, a) => {
    ...
    for (const path of await r.storage.list(`${projectPath(a)}/feedback`)) { const f = await r.storage.readJson(path); if (Object.keys(v).every(k => f.target[k] === a[k])) items.push(f); }
    return { items };
  } });
  define('resolve_feedback', { description: '显式标记用户反馈已处理。', input: input({ ...p, feedbackId: id('feedback') }), data: object({ feedback: feedbackSchema }), mutates: true, persists: true, handler: async (r, a) => {
    await readProject(r, a.projectId); const path = `${projectPath(a)}/feedback/${a.feedbackId}.json`; const feedback = await r.storage.readJson(path);
  ...
  define('record_review', { description: '记录对确切当前内容的审核；宿主转述非签名。', input: input({ ...v, contentHash: text, status: enumeration('accepted', 'changes_requested'), comment: nullable(text), evidenceReference: text }), data: object({ review: doc('review') }), mutates: true, persists: true, handler: async (r, a) => {
    ...
    await r.writeDocument('review', `${projectPath(a)}/reviews/${review.reviewId}.json`, review); return { review };
  } });
  define('get_review_status', { description: '按当前几何、规则和组件哈希判断旧审核是否已失效。', input: object(v), data: object({ status: reviewStatus, review: nullable(doc('review')) }), handler: async (r, a) => {
    ...
    for (const path of await r.storage.list(`${projectPath(a)}/reviews`)) { const review = await r.readDocument('review', path); if (Object.keys(v).every(k => review.target[k] === a[k])) items.push(review); }
  ...
  define('list_history', { description: '读取明确图标的已保存几何历史，未发布快照不作为当前内容。', input: object(i), data: object({ items: array(object({ revision, updatedAt: text })) }), handler: async (r, a) => {
    await readMatrix(r, a); const items = [];
    for (const path of await r.storage.list(historyRoot(a))) { const m = await r.readDocument('matrix', path); items.push({ revision: m.revision, updatedAt: m.updatedAt }); }
    return { items: items.sort((a, b) => b.updatedAt.localeCompare(a.updatedAt)) };
  } });
  define('get_revision', { description: '读取图标的一个历史矩阵。', input: object({ ...i, revision }), data: object({ matrix: doc('matrix') }), handler: async (r, a) => { await readMatrix(r, a); return { matrix: await r.readDocument('matrix', `${historyRoot(a)}/${a.revision}.json`) }; } });
  define('compare_revisions', { description: '比较两个历史矩阵的变体内容，不合并。', input: object({ ...i, from: revision, to: revision }), data: object({ changedVariantIds: array(identity.variantId) }), handler: async (r, a) => {
    await readMatrix(r, a); const from = await r.readDocument('matrix', `${historyRoot(a)}/${a.from}.json`); const to = await r.readDocument('matrix', `${historyRoot(a)}/${a.to}.json`);
  ...
  for (const name of ['restore_revision', 'undo', 'redo']) define(name, { description: '显式恢复指定历史版本为新保存；保留历史，不自动合并其他会话。', input: input({ ...i, revision, evidenceReference: text }), data: object({ matrix: doc('matrix') }), mutates: true, persists: true, handler: async (r, a) => {
    const matrix = touch(r, await r.readDocument('matrix', `${historyRoot(a)}/${a.revision}.json`));
  ...
  define('get_impact', { description: '枚举规则与组件可能影响的变体和任务；仅报告，不自动修改。', input: object(s), data: object({ targets: array(object(v)), primitiveIds: array(id('prim')), taskIds: array(id('task')) }), handler: async (r, a) => {
    const taskIds = []; for (const path of await r.storage.list(`${projectPath(a)}/tasks`)) { const task = await r.readDocument('task', path); if (task.target.schemeId === a.schemeId) taskIds.push(task.taskId); }
  ...
  define('prepare_export', { description: '输出明确范围的 SVG 或源快照；逐文件校验来源，不把保存当导出。', input: input({ ...v, scope: enumeration('project', 'scheme', 'icon', 'variant'), kind: enumeration('svg', 'source') }, ['projectId', 'scope', 'kind']), data: object({ delivery: doc('delivery') }), mutates: true, persists: true, handler: async (r, a, signal) => {
    ...
    const root = `${projectPath(a)}/deliveries/${delivery.exportId}`;
    ...
    if (a.kind === 'source') {
      const project=await readProject(r,a.projectId),brief=(await getBrief(r,a)).brief,vocabulary=await r.readDocument('vocabulary',`${projectPath(a)}/vocabulary.json`);
      ...
      for(const scheme of schemes){
        ...
        const rules=await readOptional(r,`${schemePath(target)}/rules.json`,null);if(rules)await add(`${prefix}/rules.json`,'application/json',JSON.stringify(rules),rules.revision);
        for(const path of await r.storage.list(schemePath(target))){
          await new Promise(resolve=>setTimeout(resolve,0));if(signal?.aborted)throw new StudioError('CANCELLED','源导出已取消，已保存文件保留。');
          const suffix=path.slice(schemePath(target).length+1),match=/^matrix\/(i-[^/]+)\.json$/.exec(suffix);
    ...
    for (const scheme of schemes) for (const target of await schemeTargets(r, { ...a, schemeId: scheme.schemeId })) {
      ...
      {
        const built = await r.storage.readJson(`${schemePath(target)}/builds/${target.iconId}/${target.variantId}/latest.json`);
  ...
  define('get_export', { description: '读取已准备交付的范围、哈希和文件清单。', input: object({ ...p, exportId: id('export') }), data: object({ delivery: doc('delivery') }), handler: async (r, a) => { await readProject(r, a.projectId); return { delivery: await r.readDocument('delivery', `${projectPath(a)}/deliveries/${a.exportId}/delivery.json`) }; } });
  define('read_artifact', { description: '只读取已登记交付物，校验字节和哈希，不开放任意文件路径。', input: object({ ...p, exportId: id('export'), artifactId: id('artifact') }), data: object({ content: longText, mediaType: text, relativePath: text }), handler: async (r, a) => {
    await readProject(r, a.projectId); const root = `${projectPath(a)}/deliveries/${a.exportId}`; const delivery = await r.readDocument('delivery', `${root}/delivery.json`);
